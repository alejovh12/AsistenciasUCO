...
- Distingue `Business contract` (semántica pública/funcional), `Infrastructure contract` (Port/provider/protocolo técnico) y `Operational contract` (claves, labels, permisos, rotación, invalidación y ambiente). Nombres de claves Azure o configuración de provider no se convierten automáticamente en contrato HTTP ni se filtran a OpenAPI.
