...
| RED → GREEN | TEST_PLAN derivado de requisito/contrato; fallo RED por la causa esperada; tests relevantes pasan; implementador no alteró RED; correcciones del tester trazadas |
...
| Persistencia | Integración DB real cuando corresponda, fixture/versión documentados y skips revisados; no esquema administrado por backend; paridad antes de retirar JDBC; sin dual-write; N+1/rollback evaluados si aplica |
| Seguridad | 401/403 y ownership relevantes cubiertos; cero secretos nuevos; cero dependencias prohibidas; no debilitar scanners, RBAC ni tests |
...
| Documentación | Normas y matriz AS-IS actualizadas; [ledger de deuda](TECHNICAL_DEBT.md) y [manuales](MANUAL_VALIDATION_LEDGER.md) actualizados; ADR si hay decisión duradera |
