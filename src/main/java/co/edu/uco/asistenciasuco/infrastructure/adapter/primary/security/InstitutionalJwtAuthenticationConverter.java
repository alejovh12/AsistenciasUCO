 ...
 *
 * <p>Depende únicamente de {@link JwtClaimsAdapter} (una SPI de infraestructura neutral), no
...
public final class InstitutionalJwtAuthenticationConverter implements Converter<Jwt, JwtAuthenticationToken> {
    ...
    public InstitutionalJwtAuthenticationConverter(final JwtClaimsAdapter jwtClaimsAdapter) {
        this.jwtClaimsAdapter = Objects.requireNonNull(jwtClaimsAdapter, "JwtClaimsAdapter es obligatorio.");
    }

    @Override
    public JwtAuthenticationToken convert(final Jwt jwt) {
        final UUID idUsuario = jwtClaimsAdapter.requireUserId(jwt);

        final Collection<GrantedAuthority> authorities = jwtClaimsAdapter.extractRoles(jwt).stream()
