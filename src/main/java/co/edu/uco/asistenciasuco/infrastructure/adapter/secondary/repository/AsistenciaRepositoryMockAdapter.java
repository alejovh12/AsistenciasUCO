...
public final class AsistenciaRepositoryMockAdapter implements AsistenciaRepositoryPort {
    ...
    @Override
    public List<AsistenciaRepositoryEntity> consultarAsistenciasPorGrupo(final ConsultarAsistenciasPorGrupoRepositoryDTO dto) {
        if (ObjectHelper.isNull(dto)) {
            throw new CrosscuttingException("El dominio para consultar asistencias por grupo es obligatorio.");
        }

        final List<AsistenciaRepositoryEntity> asistencias = new ArrayList<>();
        final UUID sesionSolicitada = ObjectHelper.isNull(dto.getSesion()) ? SESION_1 : dto.getSesion();

        asistencias.add(new AsistenciaRepositoryEntity(
                ASISTENCIA_1,
                ESTUDIANTE_1,
                dto.getGrupo(),
                sesionSolicitada,
