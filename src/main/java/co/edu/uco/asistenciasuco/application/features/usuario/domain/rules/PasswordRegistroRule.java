...
public final class PasswordRegistroRule {
    ...
    public static String resolver(
        ...
        if (numeroIdentificacion == null) {
            throw new ValidationException(UsuarioErrorCode.ERR_PASSWORD_NUMERO_IDENTIFICACION_REQUERIDO);
        }
        if (password == null) {
            return null;
        }
        if (String.valueOf(numeroIdentificacion).equals(password)) {
            throw new ValidationException(UsuarioErrorCode.ERR_PASSWORD_IGUAL_IDENTIFICACION);
        }
    ...
    public static String resolverCredencialNueva(
        ...
        if (password == null) {
            throw new ValidationException(UsuarioErrorCode.ERR_PASSWORD_REQUERIDO);
        }
    ...
    private static void validarPasswordExplicito(final String valor) {
        ...
        if (TextHelper.isNullOrBlank(valor) || TextHelper.containsWhitespace(valor) || !cumplePolitica(valor)) {
            throw new ValidationException(UsuarioErrorCode.ERR_PASSWORD_POLITICA_INVALIDA);
        }
    }

    private static void validarLongitud(final String valor) {
        if (valor.length() > MAX_LENGTH) {
            throw new ValidationException(UsuarioErrorCode.ERR_PASSWORD_LONGITUD_INVALIDA);
        }
    }
